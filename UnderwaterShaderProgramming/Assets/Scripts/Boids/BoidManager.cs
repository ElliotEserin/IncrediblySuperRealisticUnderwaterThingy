...
public class BoidManager : MonoBehaviour
{
    ...
    private void Start()
    {
        for (var i = 0; i < NumberOfBoids; ++i)
        {
            ...
            var transform = Instantiate(Prefab, spawnPoint.position + position, Quaternion.identity, this.transform);

            transform.GetComponent<Boid>().UpdateBoid(spawnPoint.position + position, InitialVelocity);
